...
# Schreger (2019). For a detailed discussion of the aggregation algorithm, please refer to that paper. 
...
# parent. See accompanying paper for details (Coppola, Maggiori, Neiman, and Schreger, 2019).
#
# Key output: output/cmns_aggregation/cmns_aggregation.dta
#
# Key variables:
...
if use_cgs_associated_issuer_file:
    cgs_ai_aggregation = pd.read_stata("aggregation_sources/cgs_ai_aggregation.dta")
else:
...
if use_factset:
    factset_aggregation = pd.read_stata("aggregation_sources/factset_aggregation.dta")
else:
...
sdc_clean = sdc_clean.merge(cgs.rename(columns={'issuer_number': 'cusip6'})[['cusip6', 'residency']], 
    on="cusip6", how="left").fillna("")
sdc_clean.loc[sdc_clean.residency.isin(tax_havens), "use_cusip"] = 1. 
sdc_clean.loc[sdc_clean.residency.isin(tax_havens), "use_up_cusip"] = 1. 
...
sdc = sdc_clean.drop(columns=['use_cusip', 'use_up_cusip', 'residency'])
...
step6 = step6.rename(columns={
    ...
    'cusip6_up_bg': 'issuer_number_up',
    'cusip6_up_bg_source': 'issuer_number_up_source',
    'country_bg': 'domicile_up',
    'country_bg_source': 'domicile_up_source'
})
...
compact_cols = ['issuer_number', 'issuer_name', 'residency', 'issuer_number_up', 'domicile_up', 
    'issuer_name_up', 'issuer_number_up_source', 'domicile_up_source']
for col in compact_cols:
    step6[col] = step6[col].astype(str).str.replace("\u039c", "")

# Store final output
step6[compact_cols].to_stata("output/cmns_aggregation/cmns_aggregation.dta", write_index=False)
