...
* The accompanying guide (CMNS_Data_Guide.pdf) provides more details on the CGS data; the primary
* output is a series of files containing essential informations about global CUSIP-bearing securities
* and their issuers.
* --------------------------------------------------------------------------------------------------
cap log close
log using "$logs/${whoami}_CGS_Build", replace

* --------------------------------------------------------------------------------------------------
* Extract zipped raw data
* --------------------------------------------------------------------------------------------------

!mkdir -p "$temp/cgs_uncompress"
!unzip "$raw/cgs_master/AIMASTER.PIP.zip -d $temp/cgs_uncompress"
!unzip "$raw/cgs_master/ALLMASTER_ISSUER.PIP.zip -d $temp/cgs_uncompress"
!unzip "$raw/cgs_master/CPMASTER_ATTRIBUTE.PIP.zip -d $temp/cgs_uncompress"
!unzip "$raw/cgs_master/CPMASTER_ISSUE.PIP.zip -d $temp/cgs_uncompress"
!unzip "$raw/cgs_master/CPMASTER_ISSUER.PIP.zip -d $temp/cgs_uncompress"
!unzip "$raw/cgs_master/FFAPlusMASTER.PIP.zip -d $temp/cgs_uncompress"
!unzip "$raw/cgs_master/INCMSTR.PIP.zip -d $temp/cgs_uncompress"
!unzip "$raw/cgs_master/TBA Master File - Sept 2012 Rev.zip" -d "$temp/cgs_uncompress"
!gunzip -c "$raw/cgs_master/ALLMASTER_ISIN.PIP.gz > $temp/cgs_uncompress/ALLMASTER_ISIN.PIP"

!mkdir -p "$temp/cgs_uncompress/delivery_2016"
!unzip "$raw/cgs_master/previous_versions/delivery_2016/AIMASTER.PIP.zip" -d "$temp/cgs_uncompress/delivery_2016"
!unzip "$raw/cgs_master/previous_versions/delivery_2016/ALLMASTER_ISSUER.PIP.zip" -d "$temp/cgs_uncompress/delivery_2016"
!unzip "$raw/cgs_master/previous_versions/delivery_2016/FFAPlusMASTER.PIP.zip" -d "$temp/cgs_uncompress/delivery_2016"
!unzip "$raw/cgs_master/previous_versions/delivery_2016/INCMSTR.PIP.zip" -d "$temp/cgs_uncompress/delivery_2016"
!gunzip -c "$raw/cgs_master/previous_versions/delivery_2016/ALLMASTER_ISIN.PIP.gz" > "$temp/cgs_uncompress/delivery_2016/ALLMASTER_ISIN.PIP"
...
* TBA
import delimited "$temp/cgs_uncompress/TBA Master File - Sept 2012 Rev.txt", encoding(ISO-8859-1) clear
replace v2=trim(v2)
split(v2), p(" ")
keep v1 v21
rename v1 cusip
...
drop add_* _merge
unique(issuer_num)
save "$aggregation_sources/cgs_compact_complete.dta", replace

* Also append with full security universe
use "$aggregation_sources/cgs_compact_complete.dta", clear
...
use issuer_num issue_num issue_check isin curr using "$temp/cgs/ALLMASTER_ISIN.dta", clear
...
mmerge ai_parent_issuer_num using "$aggregation_sources/cgs_compact_complete.dta", umatch(issuer_num) uname("ai_parent_")
drop if _merge==2
drop _merge
...
save "$aggregation_sources/cgs_ai_aggregation.dta", replace
