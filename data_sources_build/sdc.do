* --------------------------------------------------------------------------------------------------
* SDC_Build: This file build the ultimate-parent aggregation data from SDC Platinum, which is
...
mmerge CUSIP using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(cgs_)
gen use_CUSIP = 1
...
mmerge cusip6_sdc using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(cgs_)
...
mmerge ImmediateParentCUSIP using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(cgs_)
...
mmerge UltimateParentCUSIP using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(cgs_)
...
mmerge cusip6_cgs using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(cgs_)
...
save "$aggregation_sources/sdc_names", replace
...
use "$raw/figi/figi_master", clear
...
save "$temp/figi_cusip6_sectype", replace
...
foreach _field in "CUSIP" "cusip6_sdc" "ImmediateParentCUSIP" "UltimateParentCUSIP" "cusip6_cgs" {
	...
	mmerge `_field' using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(cgs_) ukeep(domicile)
	gen th_`_field' = 0
	forvalues j=1(1)10 {
			cap replace th_`_field' = 1 if (inlist(cgs_domicile,${tax_haven`j'}))
	}
	drop cgs_domicile
}
...
save "$aggregation_sources/sdc_aggregation", replace
...
save "$aggregation_sources/sdc_country.dta", replace
