* --------------------------------------------------------------------------------------------------
* Capital_IQ_Build: This file build the ultimate-parent aggregation data from Capital IQ, which is
* used as input in the CMNS aggregation procedure.
* --------------------------------------------------------------------------------------------------
cap log close
log using "$logs/${whoami}_Capital_IQ_Build", replace
...
* Pre-process
use "$temp/ciq_aggregation_full", clear
drop ïcompanyid
...
* Country
mmerge up_country using "$raw/macro/Concordances/iso2_iso3.dta", umatch(iso2) ukeep(iso3) uname("ciq_")
drop up_country
rename ciq_iso ciqup_country
drop if _merge==2
save "$temp/ciq_building_temp.dta", replace

* Perform a consistency and modal assignement (step2) for country on ciq ultimate parents
use "$temp/ciq_building_temp.dta", clear
...
replace country_cusip_nfp= 0 if (inlist(ciqup_country,$tax_haven1) | inlist(ciqup_country,$tax_haven2) | inlist(ciqup_country,$tax_haven3) | inlist(ciqup_country,$tax_haven4)| inlist(ciqup_country,$tax_haven5)| inlist(ciqup_country,$tax_haven6))
...
drop if country_cusip_count_nfp>=1 & (inlist(ciqup_country,$tax_haven1) | inlist(ciqup_country,$tax_haven2) | inlist(ciqup_country,$tax_haven3) | inlist(ciqup_country,$tax_haven4)| inlist(ciqup_country,$tax_haven5)| inlist(ciqup_country,$tax_haven6))
...
keep ciqup_cusip6 ciqup_country
save "$temp/ciq_unique_ciqup_country.dta", replace

* Merge in the ciqup unique country info
use "$temp/ciq_building_temp.dta", clear
drop ciqup_country
mmerge ciqup_cusip6 using  "$temp/ciq_unique_ciqup_country.dta", ukeep(ciqup_country)
...
save "$temp/ciq_cusip6_ultimateparents.dta", replace

* CIQ maps same cusip6 into multiple ciqup_cusip6. We select a unique match below
use "$temp/ciq_cusip6_ultimateparents.dta", clear
...
keep if n==1
drop n count
save "$temp/ciq_cusip6_ultimateparents_unique.dta", replace
...
rename companyname issuer_name
save "$aggregation_sources/ciq_names.dta", replace
...
use "$temp/ciq_cusip6_ultimateparents_unique.dta", clear
mmerge cusip6 using "$temp/figi_cusip6_sectype", unmatched(m) umatch(cusip6)
mmerge cusip6 using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(cgs_)
mmerge ciqup_cusip6 using "$temp/figi_cusip6_sectype", unmatched(m) umatch(cusip6) uname(up_)
mmerge ciqup_cusip6 using "$aggregation_sources/cgs_compact_complete", umatch(issuer_number) unmatched(m) uname(up_cgs_)
mmerge ciqup_cusip6 using "$aggregation_sources/ciq_names", umatch(cusip6) unmatched(m) uname(up_)
...
save "$temp/ciq_cusip6_ultimateparents_unique_filtered.dta", replace

* Keep only relevant variables
use "$temp/ciq_cusip6_ultimateparents_unique_filtered.dta", clear
keep ciqup_country cusip6 ciqup_cusip6 
rename ciqup_country country_ciq
rename ciqup_cusip6 up_cusip6
save "$aggregation_sources/ciq_aggregation.dta", replace
